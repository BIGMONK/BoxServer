...
public class LogUtils {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = RemoteUtils.getmContext().getPackageManager().getPackageInfo(RemoteUtils.getmContext().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && RemoteUtils.getmContext().getExternalCacheDir() != null)
                sDefaultDir = RemoteUtils.getmContext().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = RemoteUtils.getmContext().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
